...
class ActionManager():
	...
	def get_actions(self, temperature, air_humidity, soil_humidity,currentActuatorCommands):                
		watering = False
		light = False
		heating = False
		cooling = False
		...
		if 'all' in self.ruleset:
			...
			if  (currentActuatorCommands['heating'] and  self.__get_middle(minTemperature,maxTemperature) > currentTemperature) or currentTemperature < minTemperature:
				heating = True
			if  (currentActuatorCommands['cooling'] and  self.__get_middle(minTemperature,maxTemperature) < currentTemperature) or currentTemperature > maxTemperature:
				cooling = True			
			if (currentActuatorCommands['watering'] and  self.__get_middle(minSoilHumidity,maxSoilHumidity) > currentSoilHumidity) or currentSoilHumidity <= minSoilHumidity:
				watering = True			
